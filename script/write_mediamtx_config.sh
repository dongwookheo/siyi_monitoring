...
if [[ -z "$INSTALL_DIR" ]]; then
  # 인자 없으면 "현재 작업 디렉토리" 기준으로 thirdparty/mediamtx 사용
  SCRIPT_DIR="$(pwd)"
  REPO_DIR="$(dirname "$SCRIPT_DIR")"
  INSTALL_DIR="$REPO_DIR/thirdparty/mediamtx"
fi

mkdir -p "$INSTALL_DIR"

CONFIG_PATH="$INSTALL_DIR/mediamtx.yml"
...
writeQueueSize: 256  # Reduced for lower latency (SDK optimization)
# Maximum size of outgoing UDP packets.
# This can be decreased to avoid fragmentation on networks with a low UDP MTU.
udpMaxPayloadSize: 1200  # Reduced for wireless (SDK optimization)
...
rtspTransports: [udp, multicast, tcp]  # tcp
